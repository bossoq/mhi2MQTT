...
#include <MHI-AC-Ctrl/MHI-AC-Ctrl-core.h> //Main MHI Controller
...
// Local shadow state — populated from MHI SPI polling each loop
struct HVACSettings {
...
static mhi_ac::ACPower strToPower(const char *s);
static mhi_ac::ACMode strToMode(const char *s);
...
void sendWrappedHTML(String content)
{
  ...
  toSend.replace(F("_VERSION_"), mhi2mqtt_version);
...
void handleRoot()
{
  ...
  else
  {
    ...
    menuRootPage.replace("_SHOW_CONTROL_", (String)(mhi_ac::spi_state.has_received_data() && !_debugMode));
...
void handleStatus()
{
  ...
  if (mhi_ac::spi_state.has_received_data())
  {
    ...
    statusPage.replace(F("_HVAC_PROTOCOL_"), "[MHI SPI]");
  }
  else
  {
...
void handleControl()
{
  ...
  footerContent.replace("_VERSION_", mhi2mqtt_version);
  controlPage.replace("_TXT_BACK_", FPSTR(txt_back));
  controlPage.replace("_UNIT_NAME_", hostname);
  controlPage.replace("_RATE_", "60");
  controlPage.replace("_ROOMTEMP_", String(convertCelsiusToLocalUnit(currentStatus.roomTemperature, useFahrenheit)));
  ...
  controlPage.replace("_X50_PROTOCOL_", (String)false);
  ...
  controlPage.replace("_TEMP_", String(convertCelsiusToLocalUnit(settings.temperature, useFahrenheit)));
...
HVACSettings change_states(HVACSettings settings)
{
  ...
  if (update)
  {
    ...
    digitalWrite(LED_ACT, HIGH);
    playBeep(SET);
    mhi_ac::spi_state.power_set(strToPower(settings.power));
    mhi_ac::spi_state.mode_set(strToMode(settings.mode));
    mhi_ac::spi_state.target_temperature_set(settings.temperature);
...
void pollMhiState() {
    ...
    currentSettings.power          = powerToStr(mhi_ac::spi_state.power_get());
    currentSettings.mode           = modeToStr(mhi_ac::spi_state.mode_get());
    currentSettings.temperature    = mhi_ac::spi_state.target_temperature_get();
    currentSettings.fan            = fanToStr(mhi_ac::spi_state.fan_get());
    currentSettings.verticalVane   = vaneUDToStr(mhi_ac::spi_state.vanes_updown_get());
    currentSettings.horizontalVane = vaneLRToStr(mhi_ac::spi_state.vanes_leftright_get());

    currentStatus.operating       = mhi_ac::spi_state.compressor_get();
    currentStatus.roomTemperature = mhi_ac::spi_state.current_temperature_get();
    ...
    if (mhi_ac::operation_data_state.value_semaphore_take()) {
        if (mhi_ac::operation_data_state.outdoor_air_temperature_.has_value())
            currentStatus.outsideTemperature = mhi_ac::operation_data_state.outdoor_air_temperature_.get();
        if (mhi_ac::operation_data_state.indoor_u_bend_temperature_.has_value())
            currentStatus.coilTemperature = mhi_ac::operation_data_state.indoor_u_bend_temperature_.get();
        if (mhi_ac::operation_data_state.indoor_fan_speed_.has_value())
...
void addMQTTDeviceInfo(DynamicJsonDocument *JsonDocument)
{
  ...
  haConfigDevice["sw"] = "Mhi2MQTT " + String(mhi2mqtt_version);
  haConfigDevice["mdl"] = "HVAC MHI";
  haConfigDevice["mf"] = "Mitsubishi Heavy Industries";
...
void haConfig()
{
  ...
  haConfigVaneHorizontalOptions.add("1");
  haConfigVaneHorizontalOptions.add("2");
  haConfigVaneHorizontalOptions.add("3");
  haConfigVaneHorizontalOptions.add("4");
  haConfigVaneHorizontalOptions.add("5");
  haConfigVaneHorizontalOptions.add("WIDE");
  haConfigVaneHorizontalOptions.add("SPOT");
  if (others_avail_report)
  {
...
void handleButton()
{
  if (millis() > 10000)
  {
    ...
    case (shortPress):
      ...
      {
        digitalWrite(LED_ACT, HIGH);
        currentSettings.power = (strcmp(currentSettings.power, "ON") == 0) ? "OFF" : "ON";
        mhi_ac::spi_state.power_set(strToPower(currentSettings.power));
        digitalWrite(LED_ACT, LOW);
      }
      else
      {
...
void setup()
{
  ...
  pinMode(LED_PWR, OUTPUT);
  pinMode(BTN_1, INPUT);
  ledcAttach(BUZZER, 1000, 8); // Arduino 3.x: pin-based API, no channel numbers
  ledcWriteTone(BUZZER, 0);
  ...
  {
    const esp_task_wdt_config_t wdt_cfg_safe = { .timeout_ms = 60000, .idle_core_mask = 0, .trigger_panic = true };
    esp_task_wdt_reconfigure(&wdt_cfg_safe);
    safeMode();
  }
  ...
  if (initWifi())
  {
    ...
    // Enable operation data fields before init
    mhi_ac::operation_data_state.outdoor_air_temperature_.enabled = true;
    ...
    mhi_ac::Config mhi_config = {
        ...
        .sclk_pin = SCK_PIN,
        .cs_in_pin = CS_IN_PIN,
        .cs_out_pin = CS_OUT_PIN};
    auto initErr = mhi_ac::init(mhi_config);
    if (initErr != mhi_ac::InitError::Ok)
    {
      Log.ln(TAG, "MHI SPI init failed: CS loopback — verify CS_IN/CS_OUT wiring");
    }
...
void loop()
{
  ...
  if (!captive)
  {
    digitalWrite(LED_ACT, LOW);
    // Sync HVAC UNIT
    bool mhiConnected = mhi_ac::spi_state.has_received_data();
    if (!mhiConnected)
    {
      digitalWrite(LED_PWR, millis() / 1000 % 2);
      // MHI background task auto-reconnects; nothing to do here
    }
    else
    {
