...
namespace mhi_ac
{
  namespace operation_data
  {
    ...
    public:
      ...
      virtual void request(std::array<uint8_t, MHI_FRAME_LEN_LONG> &miso_frame) = 0;
    ...
    protected:
      ...
      void set_internal(Tinternal new_value)
      {
        ...
        Log.ln(TAG, "Operdata %s request fulfilled after %u", this->name(), new_age - this->age);
    ...
    public:
      ...
      void on_miso(std::array<uint8_t, MHI_FRAME_LEN_LONG> &miso_frame)
      {
        ...
        if (this->request_cycles >= OPERATION_DATA_REQUEST_TIMEOUT_CYCLES)
        {
          ...
          this->timeouts++;
          ESP_LOGI(TAG, "Timeout when requesting Operation Data %s", all[this->cycle_index]->name());
        }
      ...
      void on_mosi(const std::array<uint8_t, MHI_FRAME_LEN_LONG> &mosi_frame)
      {
        ...
        if (!match_found)
        {
          ...
          {
            ESP_LOGW(TAG, "Last error number: %u", mosi_frame[DB11]);
          }
          else if (mosi_frame[DB9] == 0x45 && (mosi_frame[DB6] & 0x80) != 0 && mosi_frame[DB10] == 0x12)
          {
            ESP_LOGW(TAG, "Count of following error operation data: %u", mosi_frame[DB11] + 4);
          }
          else if (mosi_frame[DB9] != 0xff)
          {
            ESP_LOGW(TAG, "Error: Unknown opdata: %02x %02x %02x", mosi_frame[DB9], mosi_frame[DB6], mosi_frame[DB10]);
          }
          else
          {
