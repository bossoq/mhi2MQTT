...
#define CLAMP(x, lower, upper) (MIN(upper, MAX(x, lower)))
    ...
    ACFan SpiState::fan_get() const
    {
        ...
        default:
            Log.ln(TAG, "Unknown fan speed: %i", fan_value);
    ...
    void SpiState::external_room_temperature_set(float value)
    {
        ...
        if (std::isnan(value))
        {
            ...
            Log.ln(TAG, "Disabled external room temperature sensor");
        }
        else
        {
    ...
    static void mhi_poll_task(void *arg)
    {
        ...
        while (1)
        {
            if (err)
            {
                Log.ln(TAG, "error %i . trans len: %i", err, spi_slave_trans.trans_len);
                frame_errors++;
            }
            err = 0;
            ...
            if (err)
            {
                ...
                {
                    Log.ln(TAG, "SPI transaction timeout. Is sclk_pin connected?");
                }
                else
                {
                    Log.ln(TAG, "get_trans_result error: %i", err);
                }
                continue;
            }
            // swap buffers
            ...
            if (spi_slave_trans.trans_len != MHI_FRAME_LEN_LONG * 8 && spi_slave_trans.trans_len != MHI_FRAME_LEN_SHORT * 8)
            {
                err = true;
                continue;
            }
            ...
            {
                std::copy_n(mosi_frame.begin(), spi_state.mosi_frame_snapshot_.size(),
            ...
            if (!double_frame)
            {
                ...
                {
                    Log.ln(TAG, "DB4 error %i", mosi_frame[DB4]);
                }
                ...
                if (mosi_frame[DB9] == 0x90 && (mosi_frame[DB6] & 0x80) == 0 && (mosi_frame[DB10] & 0x30) == 0x10)
                {
                    ...
                    Log.ln(TAG, "Current: %f, raw: %i, *230: %f", current, mosi_frame[DB11], current * 230);
                }
            }
        }
    }
